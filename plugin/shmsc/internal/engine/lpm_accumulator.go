...
// shmLpmPool buffers reassembled LPMs. The monolith uses a custom DIRTY
// binary-tiered pool (google.golang.org/grpc/internal/mem) whose tiers are sized
// to LPM lengths AND which skips zeroing on Get (the accumulator overwrites
// every returned byte before use, so zeroing is pure overhead). This
// self-contained engine (which must not import internal/*) uses the public
// default buffer pool: it is correctness-equivalent (also tiered; stricter
// zeroing cannot leak stale bytes), but it PAYS a runtime.memclr on every Get.
// Restoring a dirty (no-zero), LPM-tiered pool is a tracked performance
// follow-up.
var shmLpmPool = mem.DefaultBufferPool()
