...
// Begin. The start/end fields are written by Begin BEFORE
// state.Store(1) and read by drainPrefix only after observing
// state == 2 (which strictly orders after the in-flight transition),
// so start/end reads are well-defined.
type anchorSlot struct {
	start uint64       // 8B  — absolute ring offset of the held bytes' first byte
	end   uint64       // 8B  — absolute ring offset of one past the last held byte
...
//
//  1. Write slot.start / slot.end (plain, no concurrent reader yet —
//     slot.state is still 0).
